...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "colorPicked") {
    chrome.storage.local.get(['lastPickedColor']).then((result) => {
      const { r, g, b } = result.lastPickedColor.rgb;
      color.value = `linear-gradient(90deg, rgb(${r}, ${g}, ${b}) 0%, rgb(0, 0, 255) 100%)`;
    });
  }
});
...
<template>
  <Vue3ColorPicker v-model="color" theme="dark" mode="gradient" gradient-mode="linear" :showColorList="false"
    :showEyeDrop="true" />
